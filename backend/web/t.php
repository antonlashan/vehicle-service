...
define('THUMB_CACHE', dirname(__FILE__) . DIRECTORY_SEPARATOR . 'assets'.DIRECTORY_SEPARATOR. 'cache' . DIRECTORY_SEPARATOR);    // Path to cache directory (must be writeable)
if (!file_exists(THUMB_CACHE)) {
    mkdir(THUMB_CACHE, 0777);
}

define('BASE_PATH', dirname(__FILE__) . DIRECTORY_SEPARATOR . '..'.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR);    // Path to cache directory (must be writeable)
...
$crop = isset($_GET['crop']) ? max(0, min(1, $_GET['crop'])) : 0;
